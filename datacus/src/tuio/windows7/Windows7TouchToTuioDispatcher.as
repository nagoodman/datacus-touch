package tuio.windows7
{
	...
	import tuio.util.DisplayListHelper;
	...
	public class Windows7TouchToTuioDispatcher{
		...
		private function dispatchTap(event:TouchEvent):void{
			...
			target.dispatchEvent(new tuio.TouchEvent(tuio.TouchEvent.TAP, true, false, local.x, local.y, stagePos.x, stagePos.y, target, createTuioContainer(event)));
		}
		...
		private function touchBegin(event:TouchEvent):void{
			if(this.useTuioManager){
				...
				target.dispatchEvent(new tuio.TouchEvent(tuio.TouchEvent.TOUCH_DOWN, true, false, local.x, local.y, stagePos.x, stagePos.y, target, createTuioContainer(event)));
			}
		...
		private function dispatchTouchMove(event:TouchEvent):void{
			if(this.useTuioManager){
				...
				target.dispatchEvent(new tuio.TouchEvent(tuio.TouchEvent.TOUCH_MOVE, true, false, local.x, local.y, stagePos.x, stagePos.y, target, createTuioContainer(event)));
			}
		...
		private function dispatchTouchUp(event:TouchEvent):void{
			if(this.useTuioManager){
				...
				target.dispatchEvent(new tuio.TouchEvent(tuio.TouchEvent.TOUCH_UP, true, false, local.x, local.y, stagePos.x, stagePos.y, target, createTuioContainer(event)));
			}
